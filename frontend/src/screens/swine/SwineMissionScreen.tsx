/** Swine — Mission Control briefing: the strategic briefing Mission Control composes
 *  from the deterministic engines (read-only). Health insights are patterns, not
 *  diagnoses. */
import { useQuery } from "@tanstack/react-query";

import { getMissionBriefing } from "@/api/swine";
...
export default function SwineMissionScreen() {
  const { currentFarm } = useWorkspace();
  const farmId = currentFarm?.id;

  const q = useQuery({
    queryKey: ["swine-mission", farmId], queryFn: () => getMissionBriefing(farmId as string), enabled: !!farmId,
  });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <SwineSubnav active="/swine/mission" />
      <h1 className="mb-1 text-xl font-semibold text-gray-900 dark:text-gray-100">Mission Control · Swine</h1>
      {q.isLoading ? <Skeleton className="mt-4 h-48 rounded-xl" /> : (
        <>
