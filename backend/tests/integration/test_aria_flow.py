...
# Seed fixtures
#
# Insights and recommendations are generated by ARIA against real farm activity,
# so a fresh workspace has none. These tests used to skip in that case, which
# meant the dismiss/act paths were never actually exercised. Each fixture seeds
# one row on the per-test connection (rolled back at teardown) so the tests
# assert instead of skipping.
# ─────────────────────────────────────────────────────────────────────────────

@pytest_asyncio.fixture
async def seeded_insight(integration_session, workspace) -> str:
    """An active, undismissed insight on the test farm. Returns its id."""
    insight = AIInsight(
        farm_id=workspace.farm.id,
        insight_type="mortality_spike",
        severity="warning",
        title="Mortality above expected range",
        body="Mortality over the last 3 days is above the breed standard for this age.",
    )
    integration_session.add(insight)
    await integration_session.commit()
    return str(insight.id)


@pytest_asyncio.fixture
async def seeded_recommendation(integration_session, workspace) -> str:
    """A pending recommendation on the test farm. Returns its id."""
    rec = AIRecommendation(
        farm_id=workspace.farm.id,
        recommendation_type="feed_adjustment",
        title="Increase feed allocation",
        body="Current intake is below target for this flock age.",
        status="pending",
    )
...
class TestInsights:
    ...
    async def test_list_insights_returns_severity_counts(
        ...
        assert all(
            k in data
            for k in ("alert_count", "warning_count", "info_count", "reminder_count")
        )

    async def test_dismiss_insight(
        self, async_client, test_farm, seeded_insight, auth_headers_owner
    ):
        farm_id = test_farm.id
        resp = await async_client.patch(
            _insight_dismiss_url(farm_id, seeded_insight),
            headers=auth_headers_owner,
        )
        assert resp.status_code == 200
    ...
    async def test_dismissed_insight_excluded_from_default_list(
        ...
        # The seeded insight is in the default (not-dismissed) list to begin with.
        before = await async_client.get(_insights_url(farm_id), headers=auth_headers_owner)
        assert seeded_insight in [i["id"] for i in before.json()["data"]["items"]]
        ...
        # Default list (include_dismissed=false) should no longer include it.
        after = await async_client.get(_insights_url(farm_id), headers=auth_headers_owner)
        assert seeded_insight not in [i["id"] for i in after.json()["data"]["items"]]
...
class TestRecommendations:
    ...
    async def test_act_recommendation(
        self, async_client, test_farm, seeded_recommendation, auth_headers_owner
    ):
        farm_id = test_farm.id
        resp = await async_client.patch(
            _rec_action_url(farm_id, seeded_recommendation),
            json={"action": "acted"},
            headers=auth_headers_owner,
        )
        assert resp.status_code == 200
...
class TestUsageQuota:
    ...
    async def test_get_usage_returns_quota_data(
        ...
        assert resp.status_code == 200
        data = resp.json()["data"]
        assert "queries_used_this_month" in data
        assert "monthly_limit" in data
        assert "plan_name" in data
        assert isinstance(data["queries_used_this_month"], int)
