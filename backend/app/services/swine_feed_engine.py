"""
Greena — Swine Feed Efficiency Engine (Module 20, Milestone 5)

A PURE, deterministic engine: feed-consumption, cost and conversion math over the
recorded feeding log (kilograms throughout). No I/O, no mutation. Feed conversion
(FCR) is only meaningful where both feed and a positive weight gain exist over the
same window — otherwise it is honestly ``unavailable`` / ``unknown``, never a
fabricated ratio. (Weight gain arrives with the Growth milestone; until then FCR is
honestly unknown.)

Feed records are supplied as ``[{"quantity_kg": number, "cost": number|None}, ...]``.
Feed stock and purchase cost live in the platform Inventory module; this engine only
aggregates the domain feeding log.
"""
...
def feed_conversion(feed_kg: float | None, weight_gain_kg: float | None) -> dict:
    ...
    otherwise ``unavailable``. Missing inputs → ``unknown``."""
    if feed_kg is None or weight_gain_kg is None:
        return _lab(UNKNOWN, None, "Feed and weight-gain data are both required (weight arrives in Growth).")
    if weight_gain_kg <= 0:
...
def feed_summary(feed_records: list[dict], *, weight_gain_kg: float | None = None, days: int | None = None) -> dict:
    """Aggregate a feeding log into deterministic facts, plus FCR where a positive
    weight gain (kg) is supplied and an average daily intake where a day-span is
    supplied. Missing inputs are reported honestly, never fabricated."""
    ...
    return {
        ...
        "avg_daily_feed_kg": (
            _lab(CALCULATED, round(total_kg / days, 3), "total_feed_kg ÷ days.")
            if (days and days > 0) else _lab(UNKNOWN, None, "No day-span supplied.")
        ),
