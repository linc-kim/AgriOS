"""
AGRIOS — Farm Data Export Service
Generates professional PDF, Excel, and CSV exports of farm data.

Supported formats:
  PDF  — branded report with AGRIOS logo, tables, and summary stats
...
async def export_pdf(db: AsyncSession, farm_id: uuid.UUID) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=15 * mm,
        bottomMargin=20 * mm,
        title=f"AGRIOS Report — {farm_name}",
        author="AGRIOS Agricultural Operating System",
    )
    ...
    # ── Cover / Header ────────────────────────────────────────────────────────
    story.append(Paragraph("🌿 AGRIOS", ParagraphStyle(
    ...
    story.append(Spacer(1, 2 * mm))
    story.append(Paragraph(
        "This report was generated by AGRIOS — Agricultural Operating System. "
...
async def export_excel(db: AsyncSession, farm_id: uuid.UUID) -> bytes:
    ...
    wb.properties.title = f"AGRIOS Report — {farm_name}"
    wb.properties.creator = "AGRIOS Agricultural Operating System"
