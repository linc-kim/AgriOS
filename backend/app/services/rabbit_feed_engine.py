"""
Greena — Rabbit Feed Efficiency Engine (Module 17, Milestone 4)

A PURE, deterministic engine (GMIS §1.3, §5): feed-consumption and conversion
math over recorded feeding + weight-gain data. No I/O, no mutation. Feed
conversion is only meaningful where both feed and weight-gain data exist over the
same window — otherwise it is honestly ``unavailable`` (ledger CON-M4-4), never a
fabricated ratio.

Feed records are supplied as ``[{"quantity_kg": number, "cost": number|None,
"fed_on": date}, ...]``. Feed stock and purchase costs live in the platform
Inventory module; this engine only aggregates the domain feeding log.
"""
...
def feed_conversion(feed_kg: float | None, weight_gain_kg: float | None) -> dict:
    ...
    otherwise ``unavailable`` (Spec Part 4 §10 — only where sufficient data)."""
...
def feed_summary(feed_records: list[dict], weight_gain_g: float | None = None) -> dict:
    """Aggregate a feeding log into deterministic facts, and FCR where a positive
    weight gain is supplied.

    ``weight_gain_g``: total live-weight gain over the same window, in grams
    (from the Growth engine). ``None`` → FCR unknown rather than fabricated.
    """
    ...
    gain_kg = (weight_gain_g / 1000.0) if weight_gain_g is not None else None
    ...
    return {
        ...
        "feed_conversion_ratio": feed_conversion(total_kg if events else None, gain_kg),
    }
