"""
Greena — BSF Growth Planner API (Module 16, Part 7)

BSF's permission-guarded surface over the **platform** Growth Planner
(`growth_planner_service`, module='bsf'). The planner itself is cross-module; this
module supplies the BSF metric provider (imported here so it registers) and the
`BSF_GROWTH_VIEW`/`BSF_GROWTH_EDIT` guards.

Route map (prefix /farms/{farm_id}/bsf/growth):
  GET/POST  /plans                          list / create plans
  GET       /plans/{plan_id}                plan detail + progress (planned vs actual)
...
from app.services import bsf_growth_provider  # noqa: F401 — imported to register the provider
from app.services import growth_planner_service as svc

router = APIRouter(prefix="/farms/{farm_id}/bsf/growth", tags=["Black Soldier Fly"])
...
async def list_plans(
    ...
    _perm=Depends(require_permission(Permission.BSF_GROWTH_VIEW)),
):
...
async def create_plan(
    ...
    access: tuple = Depends(require_farm_access()),
    _perm=Depends(require_permission(Permission.BSF_GROWTH_EDIT)),
):
...
async def archive_plan(
    farm_id: UUID, plan_id: UUID, db: DBSession, current_user: CurrentUser,
    access: tuple = Depends(require_farm_access({"farm_owner", "farm_manager", "enterprise_owner"})),
    _perm=Depends(require_permission(Permission.BSF_GROWTH_EDIT)),
):
